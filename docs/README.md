# HarveLogix AI - Documentation

Complete documentation for the HarveLogix AI platform.

## 📚 Documentation Structure

### Getting Started
- **[QUICK_START.md](../QUICK_START.md)** - 30-second setup guide
- **[GETTING_STARTED.md](../GETTING_STARTED.md)** - Detailed getting started guide

### Project Overview
- **[README.md](../README.md)** - Main project overview
- **[PROJECT_STATUS.md](../PROJECT_STATUS.md)** - Current project status

### Implementation & Completion
- **[IMPLEMENTATION_COMPLETE.md](../IMPLEMENTATION_COMPLETE.md)** - What's been delivered
- **[PHASE1_COMPLETION.md](../PHASE1_COMPLETION.md)** - Phase 1 completion summary

### Architecture & Design
- **[ARCHITECTURE.md](./ARCHITECTURE.md)** - System architecture
- **[API.md](./API.md)** - API reference
- **[DESIGN.md](./DESIGN.md)** - Design system & UI guidelines

### Deployment & Operations
- **[DEPLOYMENT.md](./DEPLOYMENT.md)** - Deployment guide
- **[DEPLOYMENT_GUIDE.md](../DEPLOYMENT_GUIDE.md)** - Detailed deployment instructions
- **[PRODUCTION_CHECKLIST.md](../PRODUCTION_CHECKLIST.md)** - Pre-production checklist

### Security & Compliance
- **[SECURITY.md](../SECURITY.md)** - Security guidelines

### Contributing
- **[CONTRIBUTING.md](./CONTRIBUTING.md)** - Contribution guidelines

---

## 🗂️ Module Documentation

### Backend
See `backend/README.md` for:
- Backend API documentation
- Agent implementation details
- Testing guidelines

### Frontend (Web Dashboard)
See `web-dashboard/docs/DESIGN_SYSTEM.md` for:
- Frontend setup and development
- Component documentation
- Design system guidelines

See `web-dashboard/docs/DESIGN_SYSTEM.md` for:
- Color palette
- Typography system
- Component patterns
- Accessibility guidelines

### Infrastructure
See `infrastructure/` for:
- Terraform configuration
- CloudFormation templates
- Deployment scripts

---

## 🚀 Quick Navigation

**I want to...**
- Get started quickly → [QUICK_START.md](../QUICK_START.md)
- Understand the architecture → [ARCHITECTURE.md](./ARCHITECTURE.md)
- Deploy to production → [DEPLOYMENT.md](./DEPLOYMENT.md)
- Contribute code → [CONTRIBUTING.md](./CONTRIBUTING.md)
- Check security → [SECURITY.md](../SECURITY.md)
- View API reference → [API.md](./API.md)
- Understand the design → [DESIGN.md](./DESIGN.md)

---

## 📖 Reading Order

1. **First Time?** Start with [QUICK_START.md](../QUICK_START.md)
2. **Want Details?** Read [GETTING_STARTED.md](../GETTING_STARTED.md)
3. **Understanding System?** Check [ARCHITECTURE.md](./ARCHITECTURE.md)
4. **Building Features?** See [API.md](./API.md) and [DESIGN.md](./DESIGN.md)
5. **Going to Production?** Follow [DEPLOYMENT.md](./DEPLOYMENT.md)

---

## 📞 Support

- **Issues**: GitHub Issues
- **Discussions**: GitHub Discussions
- **Email**: support@harvelogix.ai

---

**Made with ❤️ for Indian Agriculture | Viksit Bharat 2047**
